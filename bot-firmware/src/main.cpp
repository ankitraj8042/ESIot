...
const int turnSpeed = 120;  // motor PWM during turns

// Navigation timing
const unsigned long kTurnTimeMs     = 500;   // ms to spin for ~90° turn (tune this!)
const unsigned long kCrossingTimeMs = 350;   // ms to drive straight past a node
const unsigned long kNodeCooldownMs = 800;   // ms cooldown after a turn before next detection
...
void setup() {
  ...
  delay(300);
  ledcWrite(LEFT_CHANNEL, 0); digitalWrite(leftMotorPin1, LOW);
  ...
  delay(300);
  ledcWrite(RIGHT_CHANNEL, 0); digitalWrite(rightMotorPin1, LOW);
...
void loop() {
  ...
  // ==========================================
  // LINE FOLLOWING + NAVIGATION STATE MACHINE
  // ==========================================
  switch (navState) {
    ...
    case NAV_LINE_FOLLOW: {
      ...
      // Node detection (only when route is active)
    ...
    case NAV_TURNING: {
      // Spin in place for kTurnTimeMs milliseconds
      if (turnDirection > 0) {
      ...
      if (millis() - turnStartMs >= kTurnTimeMs) {
        ...
        sendLog("Turn done (" + String(millis() - turnStartMs) + "ms)");
...
String popRouteCmd() {
  ...
  String cmd;
  if (comma == -1) { cmd = routeQueue; routeQueue = ""; }
  else { cmd = routeQueue.substring(0, comma); routeQueue = routeQueue.substring(comma + 1); }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (t == "ankit/bot/command") {
    if (msg == "START") {
      isRunning = true; routeQueue = ""; navState = NAV_LINE_FOLLOW;
      lastError = 0; integral = 0; lastPidMs = millis();
      nodeCooldownUntil = millis() + 500;
      sendLog("Started (line follow)"); publishNav("FOLLOWING");
    } else if (msg == "STOP") {
      isRunning = false; navState = NAV_STOPPED; routeQueue = "";
      stopMotors(); sendLog("Stopped"); publishNav("STOPPED");
    }
  }
  else if (t == "ankit/bot/route") {
    routeQueue = msg; isRunning = true; navState = NAV_LINE_FOLLOW;
    lastError = 0; integral = 0; lastPidMs = millis();
    nodeCooldownUntil = millis() + 500;
    sendLog("Route: " + msg + " — auto-started"); publishNav("ROUTE_LOADED");
  }
  ...
  else if (t == "ankit/bot/mode") { driveMode = msg; sendLog("Mode: " + driveMode); }
...
void reconnect() {
  ...
  if (client.connect("ESP32BotClient")) {
    ...
    client.subscribe("ankit/bot/command"); client.subscribe("ankit/bot/pid");
    client.subscribe("ankit/bot/speeds");  client.subscribe("ankit/bot/mode");
    client.subscribe("ankit/bot/manual");  client.subscribe("ankit/bot/route");
  } else {
